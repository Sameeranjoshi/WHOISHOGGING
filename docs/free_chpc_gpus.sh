#!/usr/bin/env bash
# free_chpc_gpus.sh - list free GPU slots on CHPC partitions you can access
# Usage:
#   ./free_chpc_gpus.sh
#   ./free_chpc_gpus.sh h100
#   ./free_chpc_gpus.sh a100
...
if [[ ${#PART_ACCOUNT[@]} -eq 0 ]]; then
  echo "No GPU partitions found in your allocations."
  exit 1
fi
...
for cluster in granite notchpeak kingspeak lonepeak; do
  sinfo -M "$cluster" -h -N -o "%N|%G|%T|%P|%c|%l" 2>/dev/null \
    ...
    | while IFS='|' read -r node gres state part cores timelimit; do
        [[ -z "${PART_ACCOUNT[$part]}" ]] && continue

        echo "$gres" | tr ',' '\n' | grep -oP 'gpu:[^:(]+:[0-9]+(?=\()' | while read -r gpuentry; do
